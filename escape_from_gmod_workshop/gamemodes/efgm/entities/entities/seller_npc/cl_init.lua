	...
	for _, ent in pairs (ents.FindByClass("seller_npc")) do
		if ent:GetPos():Distance(LocalPlayer():GetPos()) < 500 then
			...
			cam.Start3D2D(ent:GetPos() + ent:GetUp() * 76, Ang, 0.10)
				draw.SimpleTextOutlined( GM.LANG:GetString("efg.sellerru"), "trader_name", -8, 0, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 0, Color(255,255,255,255) )
			cam.End3D2D()
		end
	end
end)
